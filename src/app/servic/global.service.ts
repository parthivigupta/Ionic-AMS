...
export class GlobalService {
  ...
  getAgreemnetHeaderall() {
    var url = "http://localhost:8082/rebates.svc/AgreementHeaders" ;
    return new Promise(resolve => {
      this.http.get(url).subscribe(data => {
        resolve(data);
      },
  ...
  public getAccrualhdr(agr) {
    var url = "http://localhost:8082/rebates.svc/AccrualHeaders?$filter=AgreementNumber eq " +agr;
    return new Promise(resolve => {
      this.http.get(url).subscribe(data =>
        resolve(data));
  ...
  getTRheader(vend) {
    var url = "http://localhost:8082/rebates.svc/TR_Headers?$filter=Trparty eq " + vend;
    return new Promise(resolve => {
      this.http.get(url).subscribe(data =>
        resolve(data));
